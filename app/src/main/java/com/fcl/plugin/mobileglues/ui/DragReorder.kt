...
fun <T> DragReorderColumn(
    ...
    SideEffect { state.count = items.size }
        ...
        items.forEachIndexed { index, item ->
            ...
            Box(
                modifier = Modifier
                    ...
                    .onSizeChanged { state.setHeight(index, it.height) },
            ) {
                val handle = Modifier.pointerInput(state, index) {
                    detectDragGestures(
                        ...
                        onDragEnd = {
                            if (state.dragIndex == index) scope.launch { settle(state, haptic, onMove) }
                        },
                        onDragCancel = {
                            if (state.dragIndex == index) scope.launch { settle(state, haptic, onMove) }
                        },
...
/**
 * 松手：先把被拖的行滑到落点，滑到位了再真正重排。
 *
 * 顺序反过来的话（先重排再收动画）会跳一下——动画结束时的绘制位置正好等于重排后的
 * 布局位置，这一帧交接才是无缝的。
 */
